...
#------------------------------------    
    ...
    fcast_xgb_detrended = direct(t_target_detrended, xgboost_model, lags, f_steps, option)
    ...
    if DEBUG:
       try:
         plot_importance(fcast_xgb_detrended, height=0.9)
         plt.show()
       except:
...
#---------------
    ...
    if  int(company) >= 0:
        ...
        start_date=begin  
        end_date=end  
        direction=None # TODO read from command line
    ...
    predictions=[]
    options = ["use_shift","use_day_of_week","separate_col"]
